...
def forecast_multiple_linear_regression(train_data: pd.Series, test_len: int, seasonality: int) -> Optional[pd.Series]:
    """Generates forecasts using MLR with time trend and seasonality."""
    if len(train_data) < seasonality * 2: # Need enough data for seasonal dummies
        logging.warning(f"MLR: Not enough data ({len(train_data)}) for seasonality {seasonality}")
        return None
    try:
        df_train = train_data.to_frame(name='Value')
        # Time Trend
        df_train['time'] = np.arange(len(df_train))
        # Seasonality (Weekly - using week number)
        df_train['week_of_year'] = df_train.index.isocalendar().week
        df_train = pd.get_dummies(df_train, columns=['week_of_year'], drop_first=True, prefix='W')

        # Select features present after dummy creation
        features = ['time'] + [col for col in df_train.columns if col.startswith('W_')]
        ...
        future_dates = pd.date_range(start=train_data.index[-1] + pd.Timedelta(weeks=1), periods=test_len, freq=train_data.index.freq)
        df_test = pd.DataFrame(index=future_dates)
        df_test['time'] = np.arange(len(train_data), len(train_data) + test_len)
        df_test['week_of_year'] = df_test.index.isocalendar().week
        df_test = pd.get_dummies(df_test, columns=['week_of_year'], drop_first=True, prefix='W')

        # Align columns with training (handle missing dummy variables)
        X_test = pd.DataFrame(columns=X_train.columns, index=df_test.index).fillna(0)
        common_cols = list(set(X_test.columns) & set(df_test.columns))
        X_test[common_cols] = df_test[common_cols]
        X_test['time'] = df_test['time'] # Ensure time is always present
        X_test = X_test.fillna(0) # Fill any remaining NaNs (unlikely but safe)

        forecast_values = model.predict(X_test[X_train.columns]) # Ensure column order
